...
export default function EventDate({ config }: { config: EventConfig }) {
    ...
    <SlideWrapper>
      <div className="flex flex-col justify-center min-h-[100dvh] px-6 pt-16 pb-12 bg-white stagger md:max-w-[540px] md:mx-auto md:border-x md:border-border">
        ...
        <div className="grid grid-cols-2 gap-3">
          {config.dateOptions.map((opt) => (
            <button
              key={opt.label}
              onClick={() => handleSelect(opt.label)}
              className={`overflow-hidden rounded-2xl text-left shadow-sm active:scale-[0.97] transition-all ${answers.timeline === opt.label ? 'ring-2 ring-blue ring-offset-2 shadow-md' : ''}`}
            >
